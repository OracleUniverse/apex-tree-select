...
These features are planned for future versions of the plugin:

- Expand / Collapse state memory (remember which branches were expanded and restore them automatically).
- Improved accessibility and keyboard navigation (additional ARIA refinements and shortcuts).
- Optional drag-and-drop reordering with server-side persistence.

> Note: features such as lazy loading for large trees and per-node icons (from the `ICON` column) are already supported in the current version.
